...
#define NOMINMAX
#include <memory.h>
#include "../../Wrapper/TwitterWrapper/twitcurlurls.h"
#include "../../Wrapper/TwitterWrapper/twitcurl.h"
#include "../../Wrapper/TwitterWrapper/urlencode.h"
...
bool twitCurl::sampleStreamGet(size_t (*callbackfunction)(void *ptr, size_t size, size_t nmemb, void *s),void* callbackargument)
{
	return performStreamGet(  "https://stream.twitter.com/1/statuses/sample.json",callbackfunction ,callbackargument);
}
/*++
...
bool twitCurl::performStreamGet( const std::string& getUrl ,size_t (*callbackfunction)(void *ptr, size_t size, size_t nmemb, void *s),void* callbackargument)
{
	...
	//curl_easy_setopt( m_curlHandle, CURLOPT_HTTPGET, 1 );
	curl_easy_setopt( m_curlHandle, CURLOPT_URL, getUrl.c_str() );
	curl_easy_setopt(m_curlHandle, CURLOPT_WRITEFUNCTION, callbackfunction);
	curl_easy_setopt(m_curlHandle, CURLOPT_WRITEDATA, callbackargument);
	/* Send http request */
	if( CURLE_OK == curl_easy_perform( m_curlHandle ) )
	{
